...
?>
<?php 
//RECUPERATION NUMERO DE L'EXERCICE A FAIRE //
$exercice_id = $_GET['ex']; 
$requete_exercice = "exercice$exercice_id";
?>
<?php
//RECUPERATION ID DE LA SEANCE EN COURS //
$sc_id = $_GET['sc']; 
//RECUPERATION INFORMATION DE LA SEANCE EN COURS //
$reqsc = $bdd->prepare('SELECT * FROM seances WHERE ids = ?');
$reqsc->execute(array($sc_id));
$scinfo = $reqsc->fetch();
//PASSAGE DES INFORMATION DE LA BDD DANS VARIABLES //
...
?>
<?php
//PARAMETRES TIMER REPOS//
$heures   = 0;
$minutes  = $minutes_user;   // LES MINUTES  < 60 //
$secondes = $secondes_user;  // LES SECONDES < 60 //

//ARRIVER DU COMPTEUR A 0 //
$redirection = "sport.php?id=$userid&sc=$sc_id&ex=$exercice_id";
//CALCUL DES SECONDES //
$secondes = mktime(date("H") + $heures, date("i") + $minutes, date("s") + $secondes) - time();
?>
                                 ...
                                 <div class="iq-card">
                                    <div class="iq-card-header d-flex justify-content-between">
                                       <div class="iq-header-title">
                                             <h4 class="card-title">REPOS <font color="gray">(Vous travaillez <?= $sc_nom?>)</font></h4>
                                       </div>
                                    </div>
                                    <div class="card text-center">
                                       <div class="card-body">
                                          <?php
                                             ...
                                             if ($secondes <= 3600*24) {
                                             echo '<div id="minutes" style="font-size: 36px;"></div>';
                                             }
                                          ?>
                                       </div>
                                       <!--<div class="card-footer text-muted">Séance crée par</div>-->
                                 </div>   
                              </div>
                           </div>
                        </div>
